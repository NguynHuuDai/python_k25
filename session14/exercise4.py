...
def display_grades(records):
    ...
    for i in range(len(records)):
        ...
        print(
            f"{i + 1}. [{student['student_id']}] {student['name']} | "
            f"Toán: {student['math']} | "
...
def update_student_score(records):
    ...
    if subject == "1":
        found_student["math"] = score
        print(
            f">> Đã cập nhật điểm Toán của sinh viên '{found_student['name']}' thành {score}."
        )

    elif subject == "2":
        found_student["physics"] = score
        print(
            f">> Đã cập nhật điểm Lý của sinh viên '{found_student['name']}' thành {score}."
        )

    elif subject == "3":
        found_student["chemistry"] = score
        print(
            f">> Đã cập nhật điểm Hóa của sinh viên '{found_student['name']}' thành {score}."
        )
